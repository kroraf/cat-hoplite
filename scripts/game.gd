...
func _unhandled_input(event: InputEvent):
	if event.is_action_pressed("left_mouse_button"):
		_hex_selected(hexgrid.local_to_map(get_global_mouse_position()))
	if event is InputEventScreenTouch:
		if event.pressed:
			_hex_selected(hexgrid.local_to_map(event.position))
		else:
			# Finger was lifted from the screen
			pass
			
func _hex_selected(hex_coordinates: Vector2i) -> void:
	if unit_manager.is_player_turn() and not unit_manager.is_animation_playing:
			...
			if not actual_current_unit.is_in_motion and hex_coordinates in Navigation.get_walkable_cells(actual_current_unit):
